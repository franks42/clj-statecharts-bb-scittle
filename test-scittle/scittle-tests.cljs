(ns statecharts.scittle-tests
  "Scittle browser test runner for clj-statecharts.
   Runs core functional tests and displays results in the browser.
   Each test is wrapped in try/catch so one failure doesn't block the rest."
...
(defn run-all-tests []
  ...
  (log "Running Scittle browser tests...")

  (run-test "basic-machine" test-basic-machine)
  (run-test "guards" test-guards)
  (run-test "nested-states" test-nested-states)
  (run-test "parallel-states" test-parallel-states)
  (run-test "eventless-transitions" test-eventless-transitions)
  (run-test "simulated-clock" test-simulated-clock)
  (run-test "delayed-transitions" test-delayed-transitions)
  (run-test "delayed-cancel" test-delayed-cancel)
  (run-test "service" test-service)
  (run-test "assign" test-assign)
  (run-test "unknown-event-error" test-unknown-event-error)
  (run-test "history-states" test-history-states)
  (run-test "js-wall-clock" test-js-wall-clock)

  (let [{:keys [pass fail tests]} @test-results]
    (log (str "Scittle Results: " pass " passed, " fail " failed"))
    ...
    (let [container (js/document.getElementById "results")]
      (set! (.-innerHTML container)
            (str "<h2>clj-statecharts Scittle Test Results</h2>"
